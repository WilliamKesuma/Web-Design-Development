...
<section class="py-5">
      ...
      <div class="row gx-5">
        ...
        <main class="col-lg-6">
          <div class="ps-lg-3">
            ...
            <div class="row mb-4">
              ...
              <div class="col-12 mb-3">
                ...
                <div class="input-group mb-3" style="width: 170px;">
                    ...
                    <input type="text" id="quantityInput" class="form-control text-center border border-secondary" placeholder="1" aria-label="Example text with button addon" aria-describedby="button-addon1" value="1" />
            ...
            <a href="#!" class="btn btn-primary shadow-0 btnAddCart" data-id="{{ $data->id }}"> <i class="me-1 fa fa-shopping-basket " ></i> Add to cart </a>
  ...
  <script>
    var btns = document.querySelectorAll('.btnAddCart');
    ...
    increaseBtn.addEventListener('click', function() {
        ...
        quantityInput.value = currentQuantity + 1; // Menambahkan 1 ke quantity
    });
    ...
    decreaseBtn.addEventListener('click', function() {
        ...
        if (currentQuantity > 1) {
            quantityInput.value = currentQuantity - 1; // Mengurangi 1 dari quantity
        }
    });

    // for (const b of btns) {
    //     b.addEventListener('click', function(e) {
    //         var id = b.attributes.getNamedItem('data-id').value;
    //         fetch('{{ url("/cart") }}/' + id)
    //             .then(resp => resp.json())
    //             .then(d => {
    //                 alert(d.message)
    ...
    var addToCartBtn = document.querySelector('.btnAddCart');
    addToCartBtn.addEventListener('click', function(e) {
        var id = addToCartBtn.dataset.id; // Mendapatkan ID produk
        var quantity = parseInt(quantityInput.value); // Mendapatkan nilai quantity
        // Kirim permintaan ke server dengan ID produk dan quantity
        fetch('{{ route("product.add-to-cart", ["id" => $data->id, "name" => urlencode($data->name)]) }}', {
            method: 'POST',
            body: JSON.stringify({ id: id, quantity: quantity }), // Mengirim ID dan quantity dalam bentuk JSON
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}' // Tambahkan token CSRF untuk perlindungan formulir
            },
        })
        .then(resp => resp.json())
        .then(d => {
